-- ============================================================
-- JYOTHI FOODS ERP — 38: THE IMPORT STOPS REFUSING 250 ROWS OVER A LABEL
--
-- FORWARD ONLY. Never run a file with a lower number than one already applied.
--
-- What went wrong, from a real screen: 250 rows, 250 errors, every one of them
--
--   Unknown pack type "BOX" — add it under Setup → Pack types
--
-- Pack type is an OPTIONAL field. So is the section. Neither touches a single
-- figure: they are a label and a grouping. Yet an unrecognised one threw the
-- whole product away, and a file whose packing, rates and codes were all
-- perfectly good imported nothing at all.
--
-- The same function already creates a missing ROUTE when it meets one on a
-- customer row. There was no reason for pack types and sections to be treated
-- differently — only nobody had hit it yet.
--
-- So: p_options.create_lookups. Off by default, because creating master rows
-- from a spreadsheet is exactly how "BOX", "Box" and "BOZ" become three pack
-- types. Ticked, an unknown pack type or section is created as it appears.
-- Unticked, the refusal now says the tick box exists.
--
